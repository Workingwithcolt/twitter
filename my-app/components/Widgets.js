...
import {news,whoToFollow} from '../components/lib/static'
...
const style = {
    ...
    section: `bg-[#192734] my-6 rounded-xl oveflow-hidden `,
...
const Widgets =()=>{
    return(
        <div className={style.wrapper}>Widget
        <div className={style.searchBar}>
            <BiSearch className={style.searchIcon}/>
            <input type="text" placeholder='Search Twitter' className={style.inputBox}></input>
        </div>
                    ...
                    return(
                        <div key = {index} className={style.item}>
                            <div className={style.newsItemLeft}>
                                <div className={style.newsItemCategory}>{items.category}</div>
                                <div className={style.newsItemTitle}>{items.title}</div>
                            </div>
                            <div className={style.newsItemRight}>
                                <img src={items.image} alt={items.category} className={style.newsItemImage}/>
                            </div>
        ...
        <div className={style.section}>
            <div className={style.title}>Who to follow</div>
            {
                whoToFollow.map((item,index)=>{
                    <div key={index} className={style.item}>
                        <div>
                            <img src={item.avatar} alt={item.handle} className={item.followAvatar}/>
                        </div>
                        <div>
                        <div className={style.name}>{item.handle}</div>
                        <div className={style.handle}>{item.name}</div>
                    </div>
                    <div className={style.followAvatar}>follow</div>
                    </div>
                })
            }
        </div>
